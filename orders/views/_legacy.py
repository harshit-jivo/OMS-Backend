"""Order views not yet split into a domain module.

This file is the REMAINDER of `orders/views.py`, and is meant to shrink to
nothing. Plan item 3.1 splits that file by domain — dashboards, schemes,
parties and products, mart, order flow — one module at a time, each move
verified by the suite. `notifications.py` and `_shared.py` came out first.

Nothing new should be added here. A new view belongs in the domain module it
serves, or in a new one.
"""
