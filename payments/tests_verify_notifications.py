...
class VerificationNotificationTests(TestCase):
    ...
    def _recipients(self, event_type):
        return set(
            Notification.objects.filter(event_type=event_type)
    ...
    def test_creation_notifies_the_eligible_verifiers(self):
        ...
        self.assertEqual(
            self._recipients(PAYMENT_VERIFICATION_REQUIRED),
            {'n_verifier', 'n_verifier2'},
        )

    def test_admins_are_not_notified(self):
        receipt = _receipt('RC-N-2', self.creator)
        publish_receipt_verification_required(receipt, self.creator)
        self.assertNotIn('n_admin', self._recipients(PAYMENT_VERIFICATION_REQUIRED))
    ...
    def test_the_creator_is_never_asked_to_verify_their_own(self):
        ...
        publish_receipt_verification_required(receipt, self.creator)
        self.assertNotIn('n_creator', self._recipients(PAYMENT_VERIFICATION_REQUIRED))

    def test_no_verifier_configured_sends_nothing(self):
        self.verifier.extra_pages = []
        self.verifier.save(update_fields=['extra_pages'])
        self.other_verifier.extra_pages = []
        self.other_verifier.save(update_fields=['extra_pages'])
...
class PostedNotificationTests(TestCase):
    ...
    def _recipients(self):
        return set(
            Notification.objects.filter(event_type=PAYMENT_POSTED)
            .values_list('user__username', flat=True)
        )
    ...
    def test_posting_notifies_creator_and_verifier(self):
        ...
        self.assertEqual(self._recipients(), {'p_creator', 'p_verifier'})
    ...
    def test_the_doc_number_is_in_the_message(self):
        ...
        publish_receipt_posted(receipt)
        note = Notification.objects.filter(event_type=PAYMENT_POSTED).first()
    ...
    def test_one_notification_when_creator_verified_it(self):
        ...
        publish_receipt_posted(receipt)
        self.assertEqual(
            Notification.objects.filter(event_type=PAYMENT_POSTED).count(), 1)
    ...
    def test_no_verifier_still_notifies_the_creator(self):
        ...
        receipt = _receipt('RC-P-4', self.creator,
                           status=PaymentReceipt.Status.POSTED)
        publish_receipt_posted(receipt)
        self.assertEqual(self._recipients(), {'p_creator'})
